...
# List of years and table types
years = [i for i in range(1997, 2025)]

tables = ['totals', 'per_game', 'per_minute', 'per_poss', 'advanced', 'play-by-play', 'shooting', 'adj_shooting']
# Loop through each table and year
for table in tables:
    dfs_list = []
    for year in years:
    ...
    filename = os.path.join(parent_dir, "data/historic_data", f"historic_{table}.csv")
